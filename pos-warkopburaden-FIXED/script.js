...
// Pastikan semua gambar di area print (misal logo toko) sudah selesai di-render
// sebelum window.print() dipanggil. Sebelumnya window.print() langsung dipanggil
// setelah innerHTML di-set, jadi kalau gambar belum sempat ke-load, hasil cetak
// bisa hang / delay lama / kosong.
function printWhenReady() {
    const imgs = document.getElementById('print-area').querySelectorAll('img');
    if (imgs.length === 0) { window.print(); return; }
    let pending = imgs.length;
    let done = false;
    const finish = () => { if (!done) { done = true; window.print(); } };
    imgs.forEach(img => {
        if (img.complete) { pending--; if (pending <= 0) finish(); return; }
        img.addEventListener('load', () => { pending--; if (pending <= 0) finish(); });
        img.addEventListener('error', () => { pending--; if (pending <= 0) finish(); });
    });
    // Jaga-jaga: kalau ada gambar yang gak kunjung load/error (misal offline & belum ke-cache),
    // tetap lanjut print setelah 1.5 detik daripada macet selamanya.
...
async function processPayment() {
    ...
    printWhenReady(); db.cart = []; document.getElementById('customer-name').value=''; renderCart(); saveDB();
    if(window.innerWidth < 768) toggleCartMobile(false);
}
...
// Kompres & resize gambar sebelum disimpan sebagai base64.
// Ini penting karena foto langsung dari kamera HP bisa berukuran beberapa MB,
// dan gambar sebesar itu ikut dirender ulang tiap kali struk dicetak,
// yang bikin proses print ke print service jadi berat & lambat (bahkan bisa gagal).
function compressImage(file, maxSize, quality, callback) {
    ...
    reader.onload = function (e) {
        ...
        img.onerror = function () { callback(e.target.result); }; // fallback: pakai file asli kalau gagal diproses
        img.src = e.target.result;
    };
    reader.readAsDataURL(file);
}

function handleLogoUpload(input) { if (input.files && input.files[0]) { compressImage(input.files[0], 300, 0.7, function (dataUrl) { db.settings.logo = dataUrl; saveDB(); location.reload(); }); } }
function handleQrisUpload(input) { if (input.files && input.files[0]) { compressImage(input.files[0], 500, 0.7, function (dataUrl) { db.settings.qris = dataUrl; document.getElementById('setting-qris-preview').src = dataUrl; saveDB(); }); } }
